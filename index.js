...
client.connect((err) => {
  const ApartmentsCollection = client
    .db("family-villas")
    .collection("apartments-collection");
  const BookingsCollection = client.db("family-villas").collection("Bookings");
  app.post("/addApartment", (req, res) => {
    const newApartment = req.body;
    ApartmentsCollection.insertOne(newApartment).then((result) => {
  ...
  app.get("/apartmentSpecificData/:id", (req, res) => {
    ApartmentsCollection.findOne({ _id: ObjectID(req.params.id) }).then(
      (result) => {
        res.send(result);
      }
    );
  ...
  app.get("/adminBookingsCollection", (req, res) => {
    BookingsCollection.find({}).toArray((err, products) => {
      res.send(products);
    });
  });
  app.post("/bookApartment", (req, res) => {
    const bookApartment = req.body;
    BookingsCollection.insertOne(bookApartment).then((result) => {
      res.send(result.insertedCount > 0);
    });
  });
});
